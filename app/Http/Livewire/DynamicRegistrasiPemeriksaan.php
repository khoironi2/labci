...
class DynamicRegistrasiPemeriksaan extends Component
{
    ...
    public $totalHargaPemeriksaan = 0;

    public function updatedInput()
    {
        $this->totalHargaPemeriksaan = collect($this->input)->sum('harga');
        // Simpan total harga ke session
    session()->put('total_harga_pemeriksaan', $this->totalHargaPemeriksaan);

    // Ambil kembali total harga dari session untuk memastikan tidak 0
    $this->totalHargaPemeriksaan = session('total_harga_pemeriksaan', 0);
    }
...
{
    if ($request->session()->has('requestKeterangan')) {
        $this->input = array_filter($request->session()->get('requestKeterangan'));
    }
