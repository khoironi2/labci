...
@section('container')
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="index.html">Laborat</a></li>
                <li class="breadcrumb-item active">Proses Pemeriksaan</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
                ...
                <div class="row">
                    <div class="card">
                        <div class="card-body p-3">
                            ...
                            <form class="row g-3" method="post" action="{{ route('antrean-pemeriksaan.update', ['antrean_pemeriksaan' => $dataPemeriksaan->id]) }}" >
                                @csrf
                                @method('PUT')
                                <div class="col-md-3">
                                ...
                                <table id="myTable" class="table table-striped border-primary table-hover table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            {{-- <td>{{ $keterangan->bidang->name ?? 'N/A' }}</td> --}}
                                            <td>{{ $keterangan->metode->bidang->name ?? 'N/A' }}</td>
                                            <td>{{ $keterangan->metode->name ?? 'N/A' }}</td>
                                            <td>{{ $keterangan->parameter->parameter ?? 'N/A' }}</td>
                                            <td>{{ $keterangan->parameter->nilai_rujukan ?? 'N/A' }}</td>
                                            <td>{{ $keterangan->parameter->satuan ?? 'N/A' }}</td>
                                            <td>
                                                <input type="text" name="hasil[{{ $keterangan->id }}]"
                                                class="form-control"
                                                id="nama" >
                                            </td>
                                        </tr>
